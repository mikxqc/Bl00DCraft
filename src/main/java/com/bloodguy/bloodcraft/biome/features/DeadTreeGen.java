...
public class DeadTreeGen extends WorldGenTrees {
	...
	public boolean generate(World world, Random random, int i, int j, int k)
	{
			...
			for (int k1 = i - byte0; k1 <= i + byte0 && flag; k1++)
			{
				for (int i2 = k - byte0; i2 <= k + byte0 && flag; i2++)
				{
					if (i1 >= 0 && i1 < 256)
					{
						...
						if (i3 != Blocks.air && i3 != BloodCraftMod.desolationLeaf && i3 != Blocks.dirt && i3 != BloodCraftMod.desolationLog)
						{
							flag = false;
						}
					}
		...
		if (j1 != Blocks.dirt && j1 != BloodCraftMod.DesolatedDirt && j1 != Blocks.grass && j1 != BloodCraftMod.DesolatedGrass || j >= 256 - l - 1)
			return false;

		setBlockAndNotifyAdequately(world, i, j - 1, k, BloodCraftMod.desolationLog, l);
		byte byte1 = 3;
		int l1 = 0;
		...
		for (int j2 = j - byte1 + l; j2 <= j + l; j2++)
		{
			...
			for (int k4 = i - i4; k4 <= i + i4; k4++)
			{
				...
				for (int k5 = k - i4; k5 <= k + i4; k5++)
				{
					...
					{
						setBlockAndNotifyAdequately(world, k4, j2, k5, BloodCraftMod.desolationLeaf, field_48199_d);
					}
				}
			}
		}
		...
		for (int k2 = 0; k2 < l; k2++)
		{
			...
			if (k3 != Blocks.air && k3 != BloodCraftMod.desolationLeaf)
			{
				continue;
			}

			setBlockAndNotifyAdequately(world, i, j + k2, k, BloodCraftMod.desolationLog, field_48201_c);
		...
		if (field_48200_b)
		{
			for (int l2 = j - 3 + l; l2 <= j + l; l2++)
			{
				...
				for (int l4 = i - j4; l4 <= i + j4; l4++)
				{
					...
					{
						if (world.getBlock(l4, l2, j5) != BloodCraftMod.desolationLog)
						{
							continue;
						}
